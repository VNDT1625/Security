...
const QUICK_QUESTIONS = [
  ...
  },
  {
    ...
    question:
      "Doanh nghiệp có được chuyển dữ liệu cá nhân của khách hàng ra nước ngoài không?",
    icon: CircleHelp,
  },
  {
    ...
    question:
      "Khi thu thập log truy cập của người dùng, doanh nghiệp cần thông báo và xin đồng ý thế nào?",
    icon: LockKeyhole,
  },
] as const;
...
function findReferencedId(value: string, records: ScanRecord[]): string | null {
  ...
  const exact = records.find(
    (record) => record.id.toLowerCase() === token.toLowerCase(),
  );
...
export default function ChatPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (typeof scrollIntoView === "function") {
      scrollIntoView.call(messagesEndRef.current, {
        behavior: "smooth",
        block: "end",
      });
    }
  ...
  const mentionQuery =
    input.match(/(?:^|\s)@([A-Za-z0-9-]*)$/)?.[1]?.toLowerCase() ?? "";
      ...
      <main className={styles.page}>
        <header className={styles.pageHeader}>
          <div>
            ...
            <p>
              Hỏi pháp luật an ninh mạng hoặc giải thích kết quả cũ bằng @ID.
            </p>
          </div>
          <div className={styles.headerStatus} aria-label="Trạng thái trợ lý">
            <span>
              <span className={styles.liveDot} /> Trợ lý sẵn sàng
            </span>
            <span>
        ...
        <div className={styles.workspace}>
          <section className={styles.conversation} aria-label="Cuộc trò chuyện">
              ...
              {messages.length === 0 ? (
                <div className={styles.welcome}>
                  ...
                  {mode === "legal" ? (
                    <>
                      <p>
                        AI tự phân tích bối cảnh, đối chiếu Legal RAG và truy
                        xuất nguồn Chính phủ kèm liên kết gốc.
                      </p>
            ...
            <div className={styles.composerZone}>
              ...
              {error && (
                <div
                  className={`${styles.notice} ${styles.errorNotice}`}
                  role="alert"
                >
                  <span>{error}</span>
              ...
              {mode === "legal" && (
                <div
                  className={styles.legalContextBar}
                  aria-label="Bối cảnh pháp luật"
                >
                  <label>
                    Quốc gia
                    <select
                ...
                <div className={styles.contextChip} role="status">
                  ...
                  <span>
                    ...
                    <strong>
                      @{shortId(attachedId)}
                      {attachedRecord
                        ? ` · ${attachedRecord.type} · ${getRiskLevelForContext(attachedRecord.decision, attachedRecord.riskLevel, attachedRecord.score).label}`
                        : ""}
                    </strong>
                  </span>
                  <button
                      ...
                      {historyLoading ? (
                        ...
                        mentionMatches.map((record) => (
                          ...
                          >
                            ...
                            <b>{getRiskLevelForContext(record.decision, record.riskLevel, record.score).label}</b>
                          </button>
                        ))
                      )}
                    </div>
              ...
              {hasAssessment && (
                <div className={styles.extensionCta}>
                  <ShieldCheck />
                  <span>
                    Muốn kiểm tra nội dung mới? Hãy dùng đúng công cụ Analyze.
                  </span>
                  <Link href="/analyze">
                    Mở Analyze <ArrowUpRight />
                  </Link>
                </div>
              )}
            </div>
          ...
          <aside
            ...
            aria-label="Ngữ cảnh từ lịch sử"
          >
            <section>
              <p className={styles.railLabel}>NGỮ CẢNH TỪ LỊCH SỬ</p>
              <h2>Gắn kết quả bằng @ID</h2>
              <p>
                Chọn một kết quả đã phân tích để trợ lý giải thích hoặc trả lời
                tiếp.
              </p>
                  ...
                  {recentRecords.map((record) => (
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>
                          @{shortId(record.id)} · {record.timestamp}
                        </small>
                      </span>
                      <b>{getRiskLevelForContext(record.decision, record.riskLevel, record.score).label}</b>
                    </button>
            ...
            <section className={styles.quotaCard}>
              ...
              <Link href="/account/plan">
                Xem gói tài khoản <ArrowUpRight />
              </Link>
            </section>
          </aside>
        </div>
