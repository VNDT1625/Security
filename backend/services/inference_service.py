...
class InferenceService:
    ...
    def assess_url(
        ...
        *,
        ...
        url_lightgbm = (
            LightGBMRiskAdapter(
                lambda _features: float(pred.model_probability),
                model_version=pred.model_version,
            )
            if pred.model_probability is not None
            else None
        )
        risk = assess_risk_v2(
            v2_evidence,
        ...
        if source == "webpage":
            ...
            contextual = (
                ...
                if context_ai_mode == "off"
                    ...
                    enriched_metadata,
                    (
                        url_pred.model_probability
                        if url_pred and url_pred.model_probability is not None
                        else 0.0
                    ),
    ...
    def assess_action(
        ...
        if target_url:
            ...
            evidence = destination_assessment.evidence
        workflow_id = next(
            (
                item.split(":", 1)[1]
        ...
        rid = action_id
        summary = new_result.explanation
        if legal_reason:
